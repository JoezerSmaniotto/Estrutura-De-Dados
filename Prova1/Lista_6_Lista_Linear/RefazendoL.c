...
int incluiNaPosicao(ListaCF *lt, int pos, Dado dad){	
	if(lt->n==MAX_NODOS){ // Se esta cheia
		return LISTA_CHEIA;
	
	}else if(pos > MAX_NODOS){ 
		return POSICAO_INVALIDA;			
	
	}else if(lt->n==0 && (pos<MAX_NODOS)){ // Lista Vazia
			lt->v[pos]=dad;
			lt->n++;
			return SUCESSO;
		
	}else if(pos==(lt->n-1)){ // Ultima Posição
		lt->v[pos]=dad;
		lt->n++;
		return SUCESSO;
	}else {
...
int incluiAntes(ListaCF *lt, int cod, Dado dad){
	...
	if(lt->n==MAX_NODOS){
		...
		if(flag==0){
			...
			for(i=lt->n;i>=pos;i--){ // i aqui inicia meu vetor mais 1 por é em n e
				lt->v[i]=lt->v[i-1];		
			}
			lt->v[pos-1]=dad;
			lt->n++;
...
int excluiDaPosicao (ListaCF *lt, int pos, Dado *nodo){
	...
	if(pos > lt->n){
		...
		*nodo=lt->v[pos];
		...
		int i;
		*nodo=lt->v[pos];
		for(i=pos;i < lt->n;i++){ // Quando o Dado não fica na ultima posição
			lt->v[i]=lt->v[i+1];
		}
		lt->n--;
		return SUCESSO;
...
int excluiNodo (ListaCF *lt, int cod, Dado *nodo){
	...
	if(flag==0){ // Quando o cod não existe;
		return CODIGO_INEXISTENTE;	
	}else if((lt->n-1) == pos){ // Quando é o ultimo
		*nodo=lt->v[pos];
		lt->n--;
		return SUCESSO;
	
	}else{
		*nodo=lt->v[pos]; // Aqui quando o nodo não é ultimo
		for(i=pos; i < lt->n;i++){
			lt->v[i]=lt->v[i+1];
		}
		lt->n--;
